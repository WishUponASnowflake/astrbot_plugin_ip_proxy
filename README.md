好的，我已经将所有新增的命令、流量管理功能，以及配置项，整合到你提供的 `README.md` 最新版本中。

以下是完整的 `README.md` 内容，以代码块形式呈现：
...
* **域名白名单:** **(新增)** 仅代理白名单中的域名请求，防止滥用和意外流量消耗。
* **数据统计与持久化:** **(新增)** 实时统计和记录总/今日使用流量、请求成功/失败数、IP 使用量，数据在机器人重启后依然保留。
* **流量限制:** **(新增)** 可设定总流量限制，达到上限后自动停止代理服务。
...
* **设定总流量 `[流量大小]`**
    * **主指令:** `设定总流量 <流量大小>`
    * **示例:** `/设定总流量 5GB`, `/设定总流量 1000MB`
    * **作用:** 设置代理服务的总流量上限。当总使用流量达到此限制时，代理服务将自动停止。
