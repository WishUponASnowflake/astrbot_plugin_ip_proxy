...
让你的 AstrBot 能使用远程 HTTP 代理 API 转换为一个本地的、带有多项高级功能的智能代理服务。
...
* **API 转本地代理:** 将任何返回 `ip:port` 格式的 HTTP 代理 API 转换为一个稳定可用的本地 HTTP 代理。
* **智能 IP 管理:** 自动获取、验证、切换代理 IP，并支持根据设定的时间强制更新 IP。
* **域名白名单:** **(新增)** 仅代理预设白名单中的域名请求，防止滥用和意外流量消耗。
* **数据统计与持久化:** **(新增)** 实时统计和记录总/今日使用流量、请求成功/失败数、IP 使用量，数据在机器人重启后依然保留。
* **全指令化操作:** 通过聊天指令控制代理的启停、状态查询、IP切换及所有配置项的动态修改。
* **后台配置:** 兼容 AstrBot 的 Web 管理后台，方便进行初始化配置。
...
* **修改IP失效时间 `[秒数]`**
    * 作用: 设置IP的绝对有效时长，超时将强制更换。设置为0则关闭此功能。
